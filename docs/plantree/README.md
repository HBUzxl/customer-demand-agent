...
| [[plans/review-gating/README.md\|审核机制重构：写入即审批]] | **Done** | 用户裁决：审核改写入门禁（pending 对 Agent 不可见，批准即生效）+ 审批交互改对话内权限式弹条（去跳页，/review 降级积压总览）+ ADR-005 语义修订；现状已核实（pending 立即可检索/注入 prompt/降权未实现——审核≠门禁） | 2026-08-15 用户裁决（「按理说我通过之后才进记忆库」「直接在输入框上面弹个东西」） | 方案已定—（F1 门禁/F2 弹条/F3 ADR 修订） |
...
| [[plans/ui-copy-cleanup/README.md\|UI 文案去废话]] | **Done** | 用户裁决删三类用户可见文案：①解释内部机制（「可展开系统提示词行查看判定上下文」——ResultCard H2 整行删）②内部行话外露（ADR-016/L2/权限矩阵）③自证式说明（「点击行展开详情」功能自解释）；保留操作指引/错误信息/状态标记；首批清单已定位+开工全库再扫 | 2026-08-15 用户裁决（「把用户当傻子的这种设计，这种描述都给删掉」） | —（半天） |
