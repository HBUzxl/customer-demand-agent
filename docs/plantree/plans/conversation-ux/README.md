...
> 用户选项交互、前端系统性补课。状态：**Shaping**（问题确认，
> 方案待用户裁决）。2026-08-14 用户实测提出，同日扩充架构级发现。
...
用户裁决：F0 任务模型确认（会话串行 vs 排队并发）；F2 走截断重发
还是消息树/分叉会话；F3 是否与 missing_info 深度绑定。裁决后转
Ready 开工。F0 是地基，建议第一个做。
