...
> 状态：**Done**（C1/C2/C3 已落地 2026-08-14 一期收尾 goal；C3 编辑态按裁决不做）。
...
### C1 设置页扩为配置中心 ✅ 已落地（四个只读 API+Settings 配置中心 tab）
...
### C3 Prompt 模板配置化 ✅ 已落地（prompts/system.md 外置+快照+只读展示；编辑态不做）
...
（已收尾，2026-08-15）裁决与落地结果：C3 采用 prompts/ 目录外置形态 ✅；
C1 只读已落地 ✅；C1 **编辑态明确排除在一期外**（版本化编辑是二期候选，
见 goal objective 明确排除项）。本 plan 关闭。
