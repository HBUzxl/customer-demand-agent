...
type ModelConfig struct {
	...
	TimeoutSec  int     `json:"timeout_sec"`  // 超时秒，默认 60
}

// ToLLMConfig 转为 llm.Config（含 time.Duration）。
func (m ModelConfig) ToLLMConfig() llm.Config {
...
const (
	TaskAnalysis   TaskType = "analysis"    // 主分析
	TaskToolCall   TaskType = "tool_call"   // 工具调用
	TaskBackground TaskType = "background"  // 后台任务（预留）
)
